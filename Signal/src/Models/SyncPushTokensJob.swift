    ...
    func run() -> Promise<Void> {
        ...
        let runPromise: Promise<Void> =
            Promise { fulfill, _ in
                // TODO: PromiseKit only lets you specify the queue in `then`,
                //       so use a noop promise here so that we can specify the
                //       queues to use below.
                fulfill()
                ...
                } .then(on:DispatchQueue.global()) { (pushToken: String, voipToken: String) in
